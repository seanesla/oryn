...
export type ReactBitsAuroraProps = {
  className?: string;
  colorStops?: string[];
  amplitude?: number;
  blend?: number;
...
export function ReactBitsAurora({
  className,
  colorStops = ["#2a4cff", "#7cff67", "#2a4cff"],
  amplitude = 1.0,
  blend = 0.5,
  time,
  speed = 1.0,
}: ReactBitsAuroraProps) {
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      const actualTime = p.time ?? t * 0.01;
      program.uniforms.uTime.value = actualTime * p.speed * 0.1;
